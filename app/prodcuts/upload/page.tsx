...
import sellYourItemAction from "@/action"
// import { sellYourItemAction } from "";
...
const UploadFormPage:React.FC=()=>{
    ...
    return (
        <div className="px-12 pt-24 pb-12 min-h-screen max-w-[100rem] mx-auto flex gap-56">
            <div>
                <h2 className="text-2xl lg:text-4xl mb-4 uppercase pt-12">Sell Your Item!</h2>
                <p className="text-xl">Enter details in this form to start selling your item.</p>
            </div>
            <div className="mx-auto w-full h-full p-12 rounded-lg border-2 border-gray-500 border-opacity-10 shadow-lg bg-gray-953">
                {state?.type === 'error' && (<p className="text-lg mb-2 bg-green-951 border-2 border-gray-300 rounded-md p-2 my-4">{state.message}</p>  )}
                <form action={formAction}>
                    <div className="mb-6">
                        <label htmlFor="name" className="block mb-2">Name</label>
                        <input type="text" id="name" name="name" />
                        {state?.errors?.name && <span id="name-error" className="text-red-600 text-sm">{state.errors.name.join(',')}</span>}
                    </div>
                    <div className="mb-6">
